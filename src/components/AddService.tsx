...
const AddServiceModal: React.FC<AddServiceModalProps> = ({ open, onClose, categories, onSuccess }) => {
  ...
  const validate = (): boolean => {
    const areLanguagesValid = newService.languages.every(
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 6 }}>
          <Box sx={{ display: "flex", gap: 2 }}>
            ...
            <Button
              ...
              sx={{ backgroundColor: colors.color5, color: colors.color1, fontWeight: 'bold', px: 4, "&:hover": { backgroundColor: colors.color4 } }}
            >
